...
// Water shader by https://www.shadertoy.com/view/ltfGD7
...
vec3 water(float time) {
    ...
    vec3 ret = mix(vec3(0.0, 0.4453, 0.7305), vec3(0.0, 0.4180, 0.6758), waterlayer(uv + dist.xy));
    ret = mix(ret, vec3(0.8125, 0.9609, 0.9648), waterlayer(vec2(1.0) - uv - dist.yx));
    return ret;
}
...
void main() {
    ...
    oNormalMetallic.a = 0.0;
    oAlbedoRoughness.rgb = water(uTime);
    oAlbedoRoughness.a = 0.0;
}
